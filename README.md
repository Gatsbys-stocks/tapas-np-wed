...
## Aviso por email y caja de reservas

- El formulario de reserva pide también el **email del cliente** (debajo del teléfono).
- Cada vez que se crea una reserva (`POST /api/reservas`), el servidor envía automáticamente
  un correo de aviso al email de la empresa con los datos de la reserva. Se configura con
  variables de entorno (copia `.env.example` como `.env` y rellena tus datos):
  `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `RESERVA_EMAIL_TO`.
  Si no se configuran, la reserva se sigue guardando con normalidad, solo que no se envía el aviso.
- **`public/caja-reservas.html`** es un panel interno (pensado para el ordenador/tablet junto
  al TPV) con un botón "Abrir caja de reservas" que muestra el listado de reservas, permite
  filtrarlas por estado y confirmarlas o cancelarlas.
